...
requirejs(["jquery", "bootstrap", "bootstrap-switch", "underscore", "windows"],
  function($, _b, _bootstrap_switch, _, windows) {
    'use strict';

    const DATA_RESULT = "result";
    let resultsWithWindows = [];
    let table;
    let syncEngine;
    let device;
    ...
    $(function() {
      ...
      let windowsIncompleteModal = $("#windows-incomplete-modal");

      $(".js-close").click(function() {
        if (resultsWithWindows.every(function(resultWithWindow) {
          return resultWithWindow.result.isCompleted;
        })) {
          // All results completed
          resultsWithWindows.forEach(function(resultWithWindow) {
            resultWithWindow.window.close();
          });
          chrome.app.window.current().close();
        } else {
          console.log("some results not completed");
          windowsIncompleteModal.modal("show");
        }
      });

      $("#force-close-btn").click(function() {
        resultsWithWindows.forEach(function(resultWithWindow) {
          resultWithWindow.window.close();
        });
        chrome.app.window.current().close();        
      ...
      chrome.runtime.getBackgroundPage(function(bg) {
        bg.appInitialization.then(function() {
          ...
          resultsWithWindows = window.resultsWithWindows;
      ...
      const statusTable = {};
      statusTable.initialize = function() {
        table = $(".js-table-summary tbody");

        resultsWithWindows.forEach(function(obj) {
          table.append(createStatusRow(obj.scenario, obj.result, obj.window));
        });
      };

      function createStatusRow(scenario, result, win) {
        ...
        //     </label>
        // </div>
        var inputId = "onoff" + scenario.key + scenario.deviceSize;
        var onOffCheckBox = $("<input />", { type: "checkbox", "class": "onoffswitch-checkbox", id: inputId, name: "onoffswitch" });
        var checkboxContainer = $("<div />", { "class": "onoffswitch" }).append(
          onOffCheckBox,
          $("<label />", { "class": "onoffswitch-label", "for": inputId }).append(
            $("<span />", { "class": "onoffswitch-inner" }),
            $("<span />", { "class": "onoffswitch-switch" }))            
          );

        var winTd = $("<td />", { "class": "td-summary-window" }).append(checkboxContainer);
        ...
        // lastProgress save the last progress (range: 0 to 1)
        var progressBar = $("<div />", { "class": "progress-bar", "role": "progressbar", "aria-valuemin": "0", "aria-valuemax": 100 })
          .css({
            "width": "0%",
            "min-width": "3px"
          })
          .data("lastProgress", 0);
        var progressTd = $("<td />", { "class": "td-summary-progress" }).append(
          $("<div />", { "class": "progress" }).append(progressBar));

        win.contentWindow.addEventListener("onHidden", function() {
          onOffCheckBox.prop("checked", false);
        });
        win.onClosed.addListener(function() {
          onOffCheckBox.prop("checked", false);
          onOffCheckBox.prop("disabled", true);
          checkboxContainer.tooltip({
            title: "Window Closed",
            placement: "auto top",
            trigger: "click"
          });
          checkboxContainer.on("shown.bs.tooltip", function() {
            setTimeout(function() {
              checkboxContainer.tooltip("hide");
            }, 2000);
          });
        });

        var tr = $("<tr />").append(
          $("<td />", { "class": "td-summary-scenario-name" }).text(scenario.name),
          $("<td />", { "class": "td-summary-device-size" }).append(device.getLabel(result.deviceSize)),
          progressTd,
          winTd).data(DATA_RESULT, result);

        // Show the current status if test completed
        let showResult = function() {
          progressBar.empty();
          progressBar.removeClass("progress-bar-success progress-bar-danger");
          if (result.isCompleted) {
            progressBar.addClass(result.isPass ? "progress-bar-success" : "progress-bar-danger");
            progressBar.append($("<span />").append($("<strong />").text(result.statusDisplay)));
          }          
        };

        // When the progress moved, show the current status in text if needed
        progressBar.on("transitionend animationend", showResult);
        setProgress();
        showResult();
        ...
        function setProgress() {
          ...
          if (result.isCompleted) {
            if (progressBar.data("lastProgress") === 1) {
              // Progress bar is already at 100%, show result immediately
              showResult();
            } else {
              // Move progress bar to 100%, let transitionend event trigger showResult
              progressBar.data("lastProgress", 1);
              progressBar.css("width", "100%");
            }
          } else {
            // Move progress bar to the right location
            progressBar.data("lastProgress", result.progress);
            progressBar.css("width", (result.progress*100)+"%");
          }
        }

        // Whenever result is saved, adjust the progress bar
        result.addListener("save", function() {
          setProgress();
        });
...
define("summary/summary", function(){});
