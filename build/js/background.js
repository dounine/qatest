...
define('account',["jquery", "utils", "User", "windows", "EventEmitter"],
        function($, utils, User, windows, EventEmitter) {
  'use strict';
  ...
  var readyPromise = db.open({
    ...
    storage.addScenarioResult = function(scenarioResult) {
      return storage.getScenarioByKey(scenarioResult.scenarioKey).then(function(scenario) {
        if (scenario) {
          return server.results.add(scenarioResult).then(function(res) { return res[0]; });
        } else {
    ...
    storage.updateScenarioResult = function(scenarioResult) {
      return storage.getScenarioByKey(scenarioResult.scenarioKey).then(function(scenario) {
        if (scenario) {
          return server.results.update(scenarioResult).then(function(res) { return res[0]; });
        } else {
          return Promise.reject("Scenario does not exist");
        }
      });
    ...
    storage.addProject = function(newProject) {
      return server.projects.add(newProject).then(function(res) { return res[0]; });
    };
    storage.updateProject = function(project) {
      if (!project || !project.key) {
        throw new Error("Only persisted project can be updated");
      }
      return server.projects.update(project).then(function(res) { return res[0]; });
    };
    ...
    create: function(jqXHR) {
      ...
      if (jqXHR.responseText) {
        ...
        if (parsed.errors) {
          ...
          obj.messages = Object.keys(parsed).map(function(k) { return parsed[k]; });
        }
      }
...
define('api',["jquery", "Project", "Scenario", "apiError", "Action"],
  function($, Project, Scenario, ApiError, Action) {
      ...
      $.ajax(uri, {
        ...
        if (mapper && responsedData) {
          if (Array.isArray(responsedData)) {
            return responsedData.map(function(value) { return mapper(value); });
          } else {
...
define('core/sync_engine_projects',["underscore", "api", "./sync_engine", "Project"], function(_, api, syncEngine, Project) {
  ...
  var mergeUpProject = function(downloadedProjects, project) {
    ...
    } else if (!_.find(downloadedProjects, function(p) { return p.id === project.id; } )) {
...
define('core/sync_engine_scenarios',["underscore", "api", "./sync_engine", "Scenario"], function(_, api, syncEngine, Scenario) {
  ...
  var mergeUpScenario = function(downloadedScenarios, localScenario) {
    ...
    if (localScenario.id) {
      if (!_.find(downloadedScenarios, function(s) { return s.id === localScenario.id; })) {
...
// (function(exports) {
  ...
  TcpServer.prototype._onAccept = function (info) {
    ...
    tcpConnection.onClose(function(connection) {
      var notPurged = this.openSockets.every(function(c, index, array) {
        if (c.socketId === connection.socketId) {
          this.openSockets.splice(index, 1);
          return false; // stop the iteration immediately
        } else {
          return true;
        }
      }, this);
      ...
      if (notPurged) {
        error("Unable to purge tcp connection: " + tcpConnection.socketId);
      }
    }.bind(this));
    ...
    if (this.socketId) {
      ...
      chrome.sockets.tcp.close(this.socketId, function() {
        for (var i = 0; i < this.onCloseCallbacks.length; i++) {
          this.onCloseCallbacks[i](this);
        }
      }.bind(this));
    }
  };

  TcpConnection.prototype.onClose = function(cb) {
  ...
  // export { TcpServer, TcpConnection };

//   exports.TcpServer = TcpServer;
//   exports.TcpConnection = TcpConnection;

// })(window);
define("TcpServer", (function (global) {
    return function () {
        var ret, fn;
        return ret || global.TcpServer;
    };
}(this)));
...
define('migrations/migration_0-12',["migrationManager", "storage", "Action"],
  function(migrationManager, storage, Action) {
  'use strict';
...
chrome.app.runtime.onLaunched.addListener(function(launchData) {
  console.log("onLaunched: loaded: %s, launchData %O", loaded, launchData);
  if (loaded) {
...
requirejs([
  ...
  window.appInitialization = storage.ready().then(function() {
    ...
    return syncEngine.iterateAllScenarios().then(function(scenarios) {
      return Promise.all(
